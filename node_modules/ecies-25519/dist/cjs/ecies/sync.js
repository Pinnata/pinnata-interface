...
const sync_1 = require("@pedrouid/iso-crypto/sync");
const enc_utils_1 = require("enc-utils");
const shared_1 = require("./shared");
function getEciesKeysSync(sharedKey) {
    const hash = sync_1.sha512Sync(sharedKey);
    return { encryptionKey: shared_1.getEncryptionKey(hash), macKey: shared_1.getMacKey(hash) };
}
function encryptWithSharedKeySync(msg, sharedKey, publicKey, iv = sync_1.randomBytes(sync_1.IV_LENGTH)) {
    const { encryptionKey, macKey } = getEciesKeysSync(sharedKey);
    const ciphertext = sync_1.aesCbcEncryptSync(iv, encryptionKey, msg);
    const dataToMac = enc_utils_1.concatArrays(iv, publicKey, ciphertext);
    const mac = sync_1.hmacSha256SignSync(macKey, dataToMac);
...
function decryptWithSharedKeySync(encrypted, sharedKey) {
    ...
    const { encryptionKey, macKey } = getEciesKeysSync(sharedKey);
    const dataToMac = enc_utils_1.concatArrays(iv, publicKey, ciphertext);
    const macTest = sync_1.hmacSha256VerifySync(macKey, dataToMac, mac);
    sync_1.assert(macTest, sync_1.ERROR_BAD_MAC);
    const msg = sync_1.aesCbcDecryptSync(iv, encryptionKey, ciphertext);
    return msg;
}
